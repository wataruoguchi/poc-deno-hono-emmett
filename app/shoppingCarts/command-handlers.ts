...
export function confirmShoppingCart(command: ConfirmShoppingCart, state: ShoppingCart): ShoppingCartConfirmed {
  if (state.status !== "opened") {
    throw new IllegalStateError("Shopping cart is not opened");
  }

  const totalQuantityOfAllProductItems = Array.from(state.productItems.values()).reduce((acc, item) => acc + item, 0);
...
export function cancelShoppingCart(command: CancelShoppingCart, state: ShoppingCart): ShoppingCartCancelled {
  if (state.status !== "opened") {
    throw new IllegalStateError("Shopping cart is not opened");
  }
